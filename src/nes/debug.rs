...
impl Debug {
    ...
    pub fn trace(&self, system: &System, state: &mut SystemState, cpu_state: CpuState) {
        if state.debug.trace_instrs != 0 {
            if cpu_state.instruction_addr.is_some() {
                let trace = self.trace_instruction(system, state, cpu_state);
                let log = self.trace_instruction(system, state, cpu_state);
                eprintln!("{}", log);
    ...
    pub fn trace_instruction(
        ...
        let addr = regs.instruction_addr.unwrap();
        ...
        let mut read_pc = |state| -> u8 {
            ...
            self.peek(system, state, addr_inc)
        };
        ...
        let addr_string = match op.addressing {
            ...
            Addressing::Accumulator => format!("A = {:02X}", regs.reg_a),
            ...
            Addressing::ZeroPageOffset(Reg::X, ..) => {
                ...
                let a2 = a1.wrapping_add(regs.reg_x as u16) & 0xff;
                let v = read(state, a2);
                format!("${:02X},X @ {:04X} = {:02X}", a1, a2, v)
            }
            Addressing::ZeroPageOffset(Reg::Y, ..) => {
                let a1 = read_pc(state) as u16;
                let a2 = a1.wrapping_add(regs.reg_y as u16) & 0xff;
                let v = read(state, a2);
                format!("${:02X},X @ {:04X} = {:02X}", a1, a2, v)
            }
            Addressing::Absolute(..) => {
            ...
            Addressing::AbsoluteOffset(Reg::X, ..) => {
                ...
                let a2 = a1.wrapping_add(regs.reg_x as u16);
                let v = read(state, a2);
            ...
            Addressing::AbsoluteOffset(Reg::Y, ..) => {
                ...
                let a_high = read_pc(state);
                let a1 = ((a_high as u16) << 8) | a_low;
                let a2 = a1.wrapping_add(regs.reg_y as u16);
                let v = read(state, a2);
                format!("${:04X},X @ {:04X} = {:02X}", a1, a2, v)
            }
            ...
            Addressing::IndirectY(..) => {
                ...
                let a3 = a2.wrapping_add((regs.reg_y & 0xff) as u16);
                let v = read(state, a3);
        ...
        let reg_string = {
            format!(
                ...
                regs.reg_a, regs.reg_x, regs.reg_y, regs.reg_p, regs.reg_sp
            )
        };

        format!(
            "{}{: <10.10}{: >4.4} {: <30.30} {}",
            pc_string, instr_bytes_string, name, addr_string, reg_string
        )
    }
}
